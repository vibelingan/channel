...
test('Dianxiaomi emits one current observation per store-scoped source product', () => {
  ...
  const listing = (
    ...
    sourceVariantKey: `dianxiaomi|lazada|${storeKey}|${sku}`,
  ...
  const result = dianxiaomiObservationAdapter.toObservations({
    bundle,
    storeListings: [listing('hk', 1000, 2, 'SKU-1'), listing('sg', 1100, 3, 'sku-1')],
    observedAt: '2026-09-04T09:00:00.000Z',
  });
  ...
  assert.deepEqual(
    result.observations.map((observation) => ({
      ...
      variantSkus: observation.variants.map((variant) => variant.sku),
