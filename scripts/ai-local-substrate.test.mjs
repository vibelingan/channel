...
test('AI KB probe has a secret-safe local environment contract', () => {
  ...
  assert.match(probeEnvExample, /^KB_BASE_URL=https:\/\//m);
  assert.match(probeEnvExample, /^KB_API_KEY=$/m);
  assert.match(probeEnvExample, /^KB_WORKSPACE_SLUG=$/m);
...
test('host-side local KB setup never uses the Docker-only engine hostname', () => {
  assert.match(localAiEnvExample, /^KB_BASE_URL=http:\/\/anythingllm:3001$/m);
  assert.match(localAiEnvExample, /^KB_LOCAL_ADMIN_URL=http:\/\/127\.0\.0\.1:53001$/m);
  for (const source of [configureSource, ingestSource]) {
    assert.match(source, /process\.env\.KB_LOCAL_ADMIN_URL/);
    assert.doesNotMatch(source, /process\.env\.KB_BASE_URL/);
  }
