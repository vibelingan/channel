...
| B3 — BOM detail rows were reconstructed rather than transcribed | **Accepted and fixed** | The client teardown source proves that ClicBot and Lofree categories/cost allocations had been regrouped, while Oladance descriptions had been abbreviated. All three `bomBreakdown` tables now preserve the source grouping, amounts, and meaning in English. A new source-fidelity test pins every row and verifies each line-item sum equals `estBomCost`; a structured before/after comparison proves all non-BOM report fields remain equal. |
