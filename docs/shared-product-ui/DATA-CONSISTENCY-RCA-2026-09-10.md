...
v4 原始文件重放在真实后台完成 validate（1,074 件 / 54 页）后，先暂停以复现并验证上述
保存回归。PR #46 的完整 CI 通过后，在触发新部署前完成数据操作，避免部署与回填交错：
manifest `raw-replay-3c06d086-f84a-454a-bfe5-5afa051c2cfe` 于 13:26:57 UTC 确认为
applied / 54 页 / 1,074 observations；随后摘要刷新为 1,074 already present、0 created、0 failed。
两层独立读库都确认 1,067 件数字报价、1,070 件 MOQ；7 件无可用每件价恰好对应前述非 Piece
单位，4 件缺 MOQ 对应原文真实缺失。所有 observation 有属性及主图来源，1,073 件有详情图来源。
回填前后 1,081 商品的人工价格、分类、图片及发布状态逐字段对比差异为零，公开集合保持 11 件。

### 桌面编辑器的空白列回归

真实 1742px 桌面截图显示：虽然弹窗已加宽，Identity 与 Media 占据同一 CSS grid row，
较高的 Media 仍将 Identity 拉长并把 Content 挤到下一行。补充几何断言在 1440px 复现
Identity 最后字段下方 298px 空白，超过预期最多 32px。原因属于前端布局及验证遗漏，
不是客户需求不清：此前测试只量弹窗宽度、列位置和底部按钮，没有量列内空白与连续性。

产品编辑器改为两个独立的纵向 section stack：Identity / Content 与 Media / Pricing；
Lifecycle 仍在底部。没有复制字段、表单状态或数据请求，也没有改共享 Select/ImageManager。
320/390/768/1024/1440px 增加卡片自然高度、紧接间距和底部操作检查；桌面及手机实图均复看。
本地非 formal 浏览器 78 项、正式/public 浏览器 65 项通过；CI/CD 和线上认证验收另行记录，
不以布局测试代替业务验收。
