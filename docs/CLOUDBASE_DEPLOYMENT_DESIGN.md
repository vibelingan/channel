...
| PD-6 (resolved 2026-08-23) | P2 | Media delivery / CloudBase hosting cost | The reviewed 7.2 MB OEM factory clip remains a version-controlled static launch asset under the canonical 20 MiB exception. New/replacement videos require signed raw COS `PUT`. | Keep the current clip only while product-owned. When it is retired or moved to storage, explicitly prune `/media/oem-factory.mp4` from additive static hosting. |
...
workflow. PD-6 is resolved by the reviewed static launch exception and explicit
prune requirement. PD-7 can trail the first client review only if the release
owner explicitly accepts its operational risk.
