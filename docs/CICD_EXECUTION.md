...
| CB1 | **real — verdict reversed (2026-07-06, Context7)** | MCP `updateFunctionConfig` **replaces** env (default full-replace; `patchMode:"merge"` exists because of it; the CloudBase MCP skill says query existing + merge first "to prevent accidental overwriting"). The earlier "merge" reading of `要合并的环境变量` was wrong. Consistent with this doc's best-practice #5 + "Alternatives Rejected → Replace full env config". | Read-merge-update required for prod (query `getFunctionDetail` env → merge → union, with an explicit removal list). Low impact for test (manifest complete + authoritative). Tracked in `CICD_PRODUCTION_PLAN.md` §4 + `CICD_HARDENING_MIU02.md` §0/MIU-02.3. |
| CB2 | **verified not needed for HTTP (2026-07-06)** | Cloud-function security rules (`resourceType="function"`, `{"*":{"invoke"}}`) govern client SDK `callFunction`, NOT HTTP Access (official docs). Our functions are public via gateway `createAccess` (proven deploys). | No `managePermissions` gate for HTTP; keep smoke (public 200 / protected 401). Detail in `docs/CICD_HARDENING_MIU02.md` §0. |
