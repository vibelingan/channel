...
| CB1 | **verified not needed (2026-07-06)** | The CloudBase MCP `manageFunctions(updateFunctionConfig)` documents `envVariables` as “要合并的环境变量” (MERGE, not replace) — the deploy does not wipe console-set vars via the MCP path. | No env read-merge code. Residual (low): merge never deletes an omitted key. Evidence in `docs/CICD_HARDENING_MIU02.md` §0. |
