...
### Claude resolution — Codex finalization P1 (gate order) + P2 (doc) — 2026-07-01

- **P1 (claim before download) FIXED.** Moved the single-winner `finalizeClaim`
  gate to immediately after structural + constant-time secret verification and
  BEFORE any object download or destructive mutation. A concurrent same-secret
  loser now returns `CONFLICT` with NO storage read, NO delete, and NO row
  mutation — closing both the ~10 MiB download-amplification vector and the race
  where destructive byte-failure handling ran before the winner was known. Retry
  semantics (Codex's documented choice): a not-readable object is terminal for
  the now-consumed intent — the row is marked `failed` + the object best-effort
  deleted and the client re-uploads. Releasing the claim on a miss would be racy,
  and COS is read-after-write consistent so a miss is rare. New regression tests:
  loser reads/deletes/mutates nothing (`storage.reads == []`), the object is
  downloaded at most once per intent, and a byte-failure consumes the claim
  (proving claim-before-validation). admin 98 → 99.
- **P2 (doc inconsistency) FIXED.** `docs/IMAGE_UPLOAD_STORAGE_DESIGN.md` §20.10
  now states the `getUploadMetadata` credential cannot express
  `content-length-range`, so intent-time size is advisory and object-size
  enforcement is server-side at finalization (single-winner claim before
  download, then recompute size/SHA-256, reject/delete over-cap). The §25-2
  resolution row and the test list were corrected to match; `content-length-range`
  is retained only as a future lower-level COS/STS option.
- **Validation:** admin 99, shared 70, public-api 20, repo typecheck exit 0,
  biome clean.

### Increment 5 — `ProjectForm` direct COS POST — 2026-07-01

Moved the public OEM form off base64 `drawingData` onto the storage-upload flow
(design §20.10). `apps/site/src/components/ProjectForm.astro`:

- **Removed `readFileAsBase64`.** When a drawing is attached the browser now:
  (1) `createOemFileUploadIntent({fileName,mimeType,byteSize})` — gets a
  single-object COS credential + one-time `uploadSecret`; (2) multipart-`POST`s
  the bytes DIRECTLY to COS (server-minted fields first, then the `file` part —
  mirroring the admin `uploadImage`); (3) calls `submitProject` with
  `{...text, drawingFileId, uploadIntentId, uploadSecret}`. Bytes never transit
  the function's small JSON body cap (no more `EXCEED_MAX_PAYLOAD_SIZE`). The
  browser never holds a storage identity.
- **Advisory client validation** (server re-validates authoritatively): size ≤
  `OEM_FILE_MAX_BYTES` and `isAllowedOemExtension` (both imported from shared),
  with clear inline errors before any upload. Progress via `Uploading…` →
  `Submitting…`. Rate-limit `429` messages surface verbatim.
- **Copy:** the file input `accept` now matches `OEM_FILE_EXTENSIONS`
  (+`.webp/.iges/.dxf`) and the hint states the 10 MiB limit.
- **e2e:** `tests/e2e/mutation.spec.ts` "OEM browser submission" is unchanged and
  compatible — it is gated behind `E2E_ALLOW_MUTATION` + admin creds (runs only
  against a real CloudBase backend, where `getUploadCredential` mints creds), its
  UI interactions are identical, and its assertions (`project.drawing` set,
  `/api/files/:id` + `/api/images/:id` → 404) still hold. Local-disk uploads still
...
Review base: Claude pushed `f2bf266` (finalization P1/P2 fix) and `6ad15ae`
(`ProjectForm` direct COS upload). Codex pulled the live SSH head and reviewed
the changed handler, tests, shared policy, site form, design doc, and execution
doc against §20.10, the CloudBase SDK contract gate, and the private-media
delivery boundary.

- **Prior P1 resolved:** `verifyAndClaimOemUpload` now performs the
  `finalizeClaim` consume-once gate immediately after structural + constant-time
  secret verification and before any object download or destructive validation.
  Same-secret losers return `CONFLICT` without a storage read/delete, and tests
  prove winner-only storage access plus terminal handling for unreadable objects.
- **Prior P2 resolved:** the design doc no longer requires impossible
  `content-length-range` binding for the current `@cloudbase/node-sdk`
  `getUploadMetadata` path. The current contract is explicit: intent-time size
  is advisory, finalization claims once before download, then recomputes
  size/SHA-256 and rejects/deletes bad bytes.
- **Increment 5 review:** `ProjectForm` no longer serializes selected OEM files
  as base64. It validates file size/extension client-side as an advisory guard,
  mints a public OEM upload intent, posts the bytes directly to COS with
  server-minted fields before the `file` part, then submits only
  `drawingFileId`/`uploadIntentId`/`uploadSecret` with the text fields. This
...
- `pnpm --filter @vibelingan-channel/site typecheck` — pass, 0 errors
- `pnpm --filter @vibelingan-channel/shared typecheck` — pass
- `pnpm --filter @vibelingan-channel/shared test` — pass, 70 tests
- `pnpm --filter @vibelingan-channel/fn-public-api test` — pass, 20 tests
- `pnpm verify:cloudbase-sdk` — pass
...
- `pnpm lint` — pass
- `pnpm typecheck:e2e` — pass
- `pnpm test:e2e --list` — pass, 10 tests discovered
