...
| MIU-08 | OEM Cloud Storage upload + private delivery | in progress; groundwork accepted (sniffer, sanitizer, lifecycle, rate/pending caps); **Increment 1 (files schema + OEM contract) done**; endpoint actions next | design revision below + `ed8989a` review; sniffer `fde87ad`+`adae0d0`; rate helper `f84d6cc`+P2 fix `f23f56c`; foundation (this commit) | Move public OEM attachments off `drawingData` base64 JSON. New flow: public intent -> browser COS POST -> `submitProject` finalization -> admin-only short-TTL delivery. **Increment 1:** extended `files` collection with server-managed (readOnly) storage/lifecycle fields incl. `uploadSecretHash`; added `FileMetadataDoc`, OEM policy constants (10 MiB cap, 15 min intent TTL, 3 pending/source, 5/60s rate), `oemFileUploadSchema` (extension-allowlisted incl. CAD) + `fileExtension`/`isAllowedOemExtension`; 9 new shared tests (70 total) prove generic CRUD cannot forge storage/secret fields. Rate-limit invalid-count fail-closed finding resolved (`f23f56c`); `media-ratelimit` now clear to wire. **Remaining:** public intent action + `submitProject` finalization (secret verify + magic-byte sniff + compensation) + admin short-TTL delivery, COS `content-length-range`, and deployed 9-10 MiB ZIP smoke with no `EXCEED_MAX_PAYLOAD_SIZE`. |
