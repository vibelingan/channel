...
test('Products family tabs compose list queries, recover URL state, and prefill New', async ({
  ...
  await expect(page.getByLabel('Product Family', { exact: true })).toHaveValue('ai-gadgets');
...
test('mobile Products view exposes a full-width family select', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Products', exact: true }).click();
  const select = page.getByRole('combobox', { name: 'Product family' });
  await expect(select).toBeVisible();
  await select.selectOption('misc');
  await expect(select).toHaveValue('misc');
