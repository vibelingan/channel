...
test('blank input cannot become a phantom image through the URL resolver', () => {
  const state = createCatalogMediaState([' ', 'one'], (source) => `/resolved/${source}`);
  assert.deepEqual(state.sources, ['/resolved/one']);
});

test('late failure from a replaced source cannot advance the replacement source', () => {
  const replacement = createCatalogMediaState(['replacement', 'fallback']);
  assert.strictEqual(
    advanceFailedMedia(replacement, catalogMediaSourceId(0, 'old-product')),
    replacement,
  );
});
